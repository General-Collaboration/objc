...
class Instance {
  constructor(args) {
    if (
      args === undefined ||
      args === null ||
      !["string", "object"].includes(typeof args)
    ) {
  ...
  methodForSelector(selector) {
    ...
    return runtime.class_getInstanceMethod(
      runtime.object_getClass(this.__ptr),
      selector.__ptr
    );
  }
  ...
  call(selector, ...argv) {
    ...
    if (typeof method === "undefined" || method.isNull()) {
      throw new Error(
        `Unable to find method ${selector.name} on object ${this.description()}`
      );
    }

    const expectedNumberOfArguments =
      runtime.method_getNumberOfArguments(method);

    const argumentTypes = [...Array(expectedNumberOfArguments).keys()].map(
      (i) => {
  ...
  class() {
    return runtime.class_getName(
      this.type === "class" ? this.__ptr : this.call("class")
    );
  }
