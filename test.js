...
test.skip("inout parameters 2 (^@)", (t) => {
  ...
  const script = NSAppleScript.alloc().initWithSource_(
    'telll application "Safari" to get URL of current tab of window 1'
  );
...
test("Test possible selectors for 1 underscore", (t) => {
  ...
  const selectors = ["performAction:", "performAction_"];

  for (let permutation of new Selector("performAction_")
    .permutations()
    .map((s) => s.name)) {
...
test("Test possible selectors for 2 underscores", (t) => {
  ...
  for (let permutation of new Selector("performAction_withObject_")
    .permutations()
    .map((s) => s.name)) {
    t.true(selectors.includes(permutation));
  }
...
test("Test possible selectors for 3 underscores", (t) => {
  ...
  for (let permutation of new Selector("performAction_withObject_afterDelay_")
    .permutations()
    .map((s) => s.name)) {
    t.true(selectors.includes(permutation));
  }
...
test("Test possible selectors for method with leading underscore and no other underscores", (t) => {
  ...
  const selectors = ["_dateString", ":dateString"];

  for (let permutation of new Selector("_dateString")
    .permutations()
    .map((s) => s.name)) {
    t.true(selectors.includes(permutation));
  }
...
test("Test possible selectors for method with leading underscore and other underscores", (t) => {
  ...
  for (let permutation of new Selector("_dateStringForTimeZone_")
    .permutations()
    .map((s) => s.name)) {
    t.true(selectors.includes(permutation));
  }
...
test("Blocks: Block w/ struct parameter and inout BOOL*", (t) => {
  ...
  objc.import("AppKit");
  const { NSAttributedString, NSMutableAttributedString, NSLinkAttributeName } =
    objc;
  ...
  attrString.appendAttributedString_(
    NSAttributedString.alloc().initWithString_attributes_("def", {
      [NSLinkAttributeName]: "https://example.de",
    })
  );
  attrString.appendAttributedString_(
    NSAttributedString.alloc().initWithString_attributes_("ghi", {
      [NSLinkAttributeName]: "https://example.net",
    })
  );
  t.is(
    attrString.description().UTF8String(),
    `abc{
  ...
  let block = new objc.Block(
    ...
    objc.types.void,
    [id, NSRange, ref.refType(objc.types.char)]
  );
  attrString.enumerateAttributesInRange_options_usingBlock_(
    NSRange.new(0, attrString.length()),
    0,
    block
  );
  t.deepEqual(blockArgs, [
    // We expect the third pair to be missing, which tells us that stopping the iteration early by setting the *stop param worked.
...
test("Iterate over a NSDictionary", (t) => {
  ...
  const dictionary = NSDictionary.alloc().initWithObjects_forKeys_(
    ["1", "2", "3"],
    ["a", "b", "c"]
  );
...
test("[custom class] class methods", (t) => {
  const LKClass = objc.createClass(
    "LKClass2",
    "NSObject",
    {},
    {
...
test("[struct] pass field values in initializer", (t) => {
  ...
  t.is(
    "pretty cool",
    String(objc.ns("This is pretty cool").substringWithRange_(range))
  );
});
...
test("[type encoding parser] parse primitives", (t) => {
  for (const [key, value] of Object.entries(typeEncodingMappings)) {
    let expectedType;
    if (value == "pointer") {
      expectedType = new DataStructurePointer(
        new DataStructurePrimitive("void")
      );
    } else {
...
test("[type encoding parser] parse array / struct / union / const attribute", (t) => {
  const voidPtrType = new DataStructurePointer(
    new DataStructurePrimitive("void")
  );

  const testCases = {
    "[12^f]": new DataStructureArray(
      12,
      new DataStructurePointer(new DataStructurePrimitive("float"))
    ),
    "[12r^Q]": (() => {
      const elementType = new DataStructurePointer(
  ...
  const invalidTestCases = ["[q]", "[0q]", "{?=C", "8i"];
